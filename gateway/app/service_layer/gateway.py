...
from app.domain.events.auth_service import UserCreated
...
async def get_users(service: Service, client: AsyncHttpClient) -> tuple[dict[str, Any], int]:
    """
    Get users.
    ...
    return await gateway(service_url=service.base_url, path=f"{api_v1_url}/users",
                         client=client, method="GET")


async def verify_user_existence(username: str,
                                service: Service,
                                client: AsyncHttpClient) -> UserCreated:
    """
    ...
    response_event = ResponseModel[UserCreated](**response)
